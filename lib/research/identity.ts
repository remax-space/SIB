...
export function fingerprint(plan: Plan, connectionVersion: string, contextVersion: string): string {
  return hash({ scope: 'sib-authenticated-shared', contract: CONTRACT_VERSION, provider: plan.provider, connectionVersion, contextVersion, tool: plan.tool, parameters: providerParameters(plan) })
}
