...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const context = JSON.stringify({ case: analysis.case, mission: analysis.missionLiteral, basile: comparisonResult(analysis.basileResult), advocado: comparisonResult(analysis.advocadoResult), cabeca: comparisonResult(analysis.cabecaResult), auditor: comparisonResult(analysis.auditorResult), mestre: comparisonResult(analysis.mestreResult), orientador: comparisonResult(analysis.orientacoesResult), jurisprudencia: analysis.jurisprudenciaResult, docs: docs.map((doc) => ({ id: doc.id, filename: doc.filename, text: documentary ? undefined : doc.extractedText, readStatus: doc.readStatus, textSource: doc.textSource, incomplete: doc.textSource !== 'FULL_TEXT' || doc.readStatus !== 'LIDO_INTEGRALMENTE' })) })
