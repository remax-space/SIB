...
export function LegalResearch({ analysisId, initialQuery = '', onEvidence }: { analysisId: string; initialQuery?: string; onEvidence?: (evidence: Evidence) => void }) {
  ...
  return <Card id="pesquisa-juridica" className="border-primary/30">
    <CardHeader><CardTitle className="text-lg">Pesquisa jurídica — {provider === 'legaw' ? 'Legaw' : 'provedor legado'}</CardTitle><p className="text-sm text-muted-foreground">Prepare uma consulta após a análise. Ler o histórico e selecionar fontes não faz pesquisa externa.</p></CardHeader>
    <CardContent className="space-y-4">
      {error && <p role="alert" className="text-sm text-destructive">{error}</p>}
      {notice && <p role="status" className="text-sm">{notice}</p>}
      {!context ? <p>Carregando histórico…</p> : <>
        ...
        {opened && <form className="space-y-4" onChange={() => setPrepared(null)} onSubmit={e => { e.preventDefault(); void act(async () => setPrepared(await api('prepare', plan()))) }}>
          ...
          <label className="block text-sm">Objetivo (critério local)<Input required minLength={5} maxLength={1000} value={objective} onChange={e => setObjective(e.target.value)} /></label>
          {(tool === 'buscar_jurisprudencia' || tool === 'buscar_legislacao') && <label className="block text-sm">Pergunta jurídica que será enviada<Textarea required minLength={5} maxLength={2000} value={query} onChange={e => setQuery(e.target.value)} /></label>}
          ...
          <details className="border rounded p-3"><summary className="text-sm">Critérios para interpretação local (não são filtros do provedor)</summary><div className="space-y-3 mt-3"><label className="block text-sm">Fundamentos<select className="block w-full border rounded p-2 bg-background" value={stance} onChange={e => setStance(e.target.value as Plan['stance'])}><option value="ambos">Favoráveis e contrários</option><option value="favoraveis">Favoráveis</option><option value="contrarios">Contrários</option></select></label><label className="block text-sm">Termos obrigatórios<Input maxLength={500} value={requiredTerms} onChange={e => setRequiredTerms(e.target.value)} /></label><label className="block text-sm">Exclusões<Input maxLength={500} value={exclusions} onChange={e => setExclusions(e.target.value)} /></label></div></details>
        ...
        {prepared && <section className="rounded border p-4 space-y-3" aria-label="Revisão da pesquisa">
          ...
          <p className="text-sm">Dados que sairão do SIB:</p><pre className="text-xs whitespace-pre-wrap break-words bg-muted p-3 rounded">{JSON.stringify(prepared.research.parameters, null, 2)}</pre>
          <p className="text-sm">Objetivo local: {prepared.research.plan.objective}. Fundamentos: {prepared.research.plan.stance}. Termos: {prepared.research.plan.requiredTerms || 'nenhum'}. Exclusões: {prepared.research.plan.exclusions || 'nenhuma'}.</p>
