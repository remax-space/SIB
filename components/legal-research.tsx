...
import type { FullTextDefaults } from '@/lib/research/full-text-defaults'
import type { PublicResearch, PublicResearchResult as Result, PublicEvidence as Evidence } from '@/lib/research/public'
type Context = { fullTextDefaults: FullTextDefaults; integrated: { id: string; createdAt: number; sources: { id: string; title: string }[] }[]; caseId: string; cutoffDate: string | null; suggestion: string; completed: boolean; canReconcile: boolean; history: PublicResearch[]; limitations: string[]; availability: { available: boolean; legacyAvailable: boolean; blockers: string[] } }
...
type LegalResearchProps = { analysisId: string; initialQuery?: string; onEvidence?: (evidence: Evidence) => void }
export function LegalResearch(props: LegalResearchProps) {
  return <LegalResearchForm key={props.analysisId} {...props} />
}
function LegalResearchForm({ analysisId, initialQuery = '', onEvidence }: LegalResearchProps) {
  ...
  const [fullTextInitialized, setFullTextInitialized] = useState(false)
  ...
  function selectTool(next: Plan['tool']) {
    setTool(next)
    setLimit(next === 'buscar_legislacao' ? 5 : 10)
    if (next === 'ler_inteiro_teor' && !fullTextInitialized && context) {
      const defaults = context.fullTextDefaults
      setProcessNumber(defaults.processNumber ?? '')
      setTribunal(defaults.tribunal ?? '')
      setPage(defaults.page)
      setRelator(defaults.relator ?? '')
      setJudgmentDate(defaults.judgmentDate ?? '')
      setFullTextInitialized(true)
    }
  }
  function plan(): Plan {
      ...
      {!context ? error ? <Button variant="outline" disabled={busy} onClick={() => void act(async () => { const data = await reload(); setObjective(data.suggestion); setEndDate(data.cutoffDate?.slice(0, 10) ?? '') })}>Tentar carregar histórico novamente</Button> : <p>Carregando histórico…</p> : <>
        ...
        {opened && <form className="space-y-4" onChange={() => setPrepared(null)} onSubmit={e => { e.preventDefault(); void act(async () => setPrepared(await api('prepare', plan()))) }}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {provider === 'legaw' && <label className="text-sm">Tipo de fonte<select className="block w-full border rounded p-2 bg-background" value={tool} onChange={e => selectTool(e.target.value as Plan['tool'])}><option value="buscar_jurisprudencia">Jurisprudência</option><option value="buscar_legislacao">Legislação federal</option><option value="ler_inteiro_teor">Inteiro teor de acórdão</option><option value="conferir_citacoes">Conferência de citações</option></select></label>}
          </div>
        ...
        {prepared && <section className="rounded border p-4 space-y-3" aria-label="Revisão da pesquisa">
          <h3 className="font-semibold">Confira antes de pesquisar</h3><p className="text-sm">Tipo de pesquisa: {toolLabel(prepared.research.plan.tool)}. {prepared.research.plan.tool === 'ler_inteiro_teor' ? 'Será consultada uma página do inteiro teor de um acórdão, usando os parâmetros abaixo. Esta consulta não baixa o PDF atualizado dos autos completos.' : prepared.research.plan.tool === 'conferir_citacoes' ? 'Será feita uma única conferência do texto revisado abaixo.' : 'Será feita uma única consulta, com o contexto do processo resumido.'}</p>
